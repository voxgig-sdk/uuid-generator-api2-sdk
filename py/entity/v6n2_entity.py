...
class V6n2Entity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return V6n2Entity(self._client, opts)
    ...
    def match_get(self) -> V6n2:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> V6n2:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.V6n2().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
