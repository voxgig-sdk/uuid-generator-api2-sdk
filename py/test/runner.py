# UuidGeneratorApi2 SDK test runner
...
class UuidGeneratorApi2TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UuidGeneratorApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UuidGeneratorApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UuidGeneratorApi2TestRunner.getenv("UUID_GENERATOR_API2_TEST_LIVE")
        override = UuidGeneratorApi2TestRunner.getenv("UUID_GENERATOR_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UuidGeneratorApi2TestRunner.getenv(key)
        ...
        explain = UuidGeneratorApi2TestRunner.getenv("UUID_GENERATOR_API2_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["UUID_GENERATOR_API2_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if UuidGeneratorApi2TestRunner._test_control is not None:
            return UuidGeneratorApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UuidGeneratorApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UuidGeneratorApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UuidGeneratorApi2TestRunner.load_test_control()
...
def load_env_local():
    UuidGeneratorApi2TestRunner.load_env_local()


def env_override(m):
    return UuidGeneratorApi2TestRunner.env_override(m)


def entity_data(v):
    return UuidGeneratorApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UuidGeneratorApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UuidGeneratorApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UuidGeneratorApi2TestRunner.load_test_control()


def live_client_options():
    return UuidGeneratorApi2TestRunner.live_client_options()


def live_delay_ms():
    return UuidGeneratorApi2TestRunner.live_delay_ms()
