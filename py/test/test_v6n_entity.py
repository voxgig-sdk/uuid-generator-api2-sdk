...
class TestV6nEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.V6n(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.V6n(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set UUID_GENERATOR_API2_TEST_V6N_ENTID JSON to run live")
        client = setup["client"]
        ...
        v6n_ref01_list_result = v6n_ref01_ent.list(v6n_ref01_match, None)
        assert isinstance(v6n_ref01_list_result, list)

        # LOAD
        v6n_ref01_match_dt0 = {}
        v6n_ref01_data_dt0_loaded = v6n_ref01_ent.load(v6n_ref01_match_dt0, None)
        assert v6n_ref01_data_dt0_loaded is not None
...
def _v6n_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("UUID_GENERATOR_API2_TEST_V6N_ENTID"))
