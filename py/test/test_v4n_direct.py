...
class TestV4nDirect:
    ...
    def test_should_direct_list_v4n(self):
        setup = _v4n_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-v4n", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_v4n(self):
        setup = _v4n_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-v4n", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
