...
class TestV1n2Entity:

    def test_should_create_instance(self):
        testsdk = UuidGeneratorApi2SDK.test(None, None)
        ent = testsdk.V1n2(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _v1n2_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "v1n2." + _op, "live" if _live else "unit")
            if _skip:
        ...
        v1n2_ref01_data = None
        if len(v1n2_ref01_data_raw) > 0:
            v1n2_ref01_data = helpers.to_map(v1n2_ref01_data_raw[0][1])

        # LOAD
        v1n2_ref01_ent = client.V1n2(None)
        v1n2_ref01_match_dt0 = {}
        v1n2_ref01_data_dt0_loaded, err = v1n2_ref01_ent.load(v1n2_ref01_match_dt0, None)
        assert err is None
        assert v1n2_ref01_data_dt0_loaded is not None



def _v1n2_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/v1n2/V1n2TestData.json")
