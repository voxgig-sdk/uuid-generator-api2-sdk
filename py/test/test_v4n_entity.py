...
class TestV4nEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.V4n(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.V4n(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _v4n_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "v4n." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set UUID_GENERATOR_API2_TEST_V4N_ENTID JSON to run live")
        client = setup["client"]
        ...
        v4n_ref01_data = None
        if len(v4n_ref01_data_raw) > 0:
            v4n_ref01_data = helpers.to_map(v4n_ref01_data_raw[0][1])

        # LIST
        v4n_ref01_ent = client.V4n(None)
        v4n_ref01_match = {}

        v4n_ref01_list_result = v4n_ref01_ent.list(v4n_ref01_match, None)
        assert isinstance(v4n_ref01_list_result, list)

        # LOAD
        v4n_ref01_match_dt0 = {}
        v4n_ref01_data_dt0_loaded = v4n_ref01_ent.load(v4n_ref01_match_dt0, None)
        assert v4n_ref01_data_dt0_loaded is not None



def _v4n_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/v4n/V4nTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "UUID_GENERATOR_API2_TEST_V4N_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("UUID_GENERATOR_API2_TEST_V4N_ENTID"))
