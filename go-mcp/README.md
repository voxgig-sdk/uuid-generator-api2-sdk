# uuid-generator-api2-mcp

[MCP](https://modelcontextprotocol.io) server exposing the UuidGeneratorApi2 SDK as
two agent tools — `uuid-generator-api2_list` and `uuid-generator-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/uuid-generator-api2-mcp)
make build

# 2. Provide credentials via the environment
export UUID_GENERATOR_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user uuid-generator-api2 \
  -- /absolute/path/to/uuid-generator-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./uuid-generator-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// uuid-generator-api2_list: first page of records
{ "entity": "guid" }
{ "entity": "guid", "query": { } }

// uuid-generator-api2_load: one record by id
{ "entity": "guid", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/uuid-generator-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user uuid-generator-api2 \
     -- "$PWD"/dist/*/uuid-generator-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `uuid-generator-api2_list` and `uuid-generator-api2_load` tools now appear
   in new sessions. Ask the agent to *"list guid using uuid-generator-api2"*
   and it calls `uuid-generator-api2_list` with `{"entity":"guid"}`.
...
```sh
export UUID_GENERATOR_API2_APIKEY=sk_live_xxx            # API key
export UUID_GENERATOR_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `uuid-generator-api2_list` tool
...
### Call the `uuid-generator-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `uuid-generator-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `uuid-generator-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `UUID_GENERATOR_API2_APIKEY` | API key sent with every request. |
| `UUID_GENERATOR_API2_BASE` | Optional override of the API base URL. |
...
```sh
./uuid-generator-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"uuid-generator-api2_load","arguments":{"entity":"guid","query":{"id":1}}}}'
```
