...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.UUIDGENERATORAPI2_TEST_LIVE ||
    'TRUE' === process.env.UUIDGENERATORAPI2_TEST_OVERRIDE
  ) {
  ...
  m.UUIDGENERATORAPI2_TEST_EXPLAIN = process.env.UUIDGENERATORAPI2_TEST_EXPLAIN || m.UUIDGENERATORAPI2_TEST_EXPLAIN
