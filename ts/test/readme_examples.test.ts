...
//     (\`new UuidGeneratorApi2SDK(...)\` / \`UuidGeneratorApi2SDK.test(...)\` become
//     \`UuidGeneratorApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'UuidGeneratorApi2SDK'
...
// rewrite real client construction (\`new UuidGeneratorApi2SDK(...)\` and any
// \`UuidGeneratorApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UuidGeneratorApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UuidGeneratorApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
