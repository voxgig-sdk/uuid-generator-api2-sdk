...
describe('V6nDirect', async () => {
  ...
  test('direct-load-v6n', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-v6n', setup.live)) return
  ...
  test('direct-list-v6n', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-v6n', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['UUID_GENERATOR_API2_TEST_V6N_ENTID']
