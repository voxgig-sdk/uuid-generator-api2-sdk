...
describe('GuidDirect', async () => {
  ...
  test('direct-load-guid', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-guid', setup.live)) return
  ...
  test('direct-list-guid', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-guid', setup.live)) return
    const { client, calls } = setup
