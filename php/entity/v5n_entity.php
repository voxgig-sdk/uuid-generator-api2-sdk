...
// UuidGeneratorApi2 SDK V5n entity
...
class V5nEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new V5nEntity($this->_client, $opts);
    }

    /**
     * @param V5n|array $args V5n data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = UuidGeneratorApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return V5n|array The current V5n data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of V5n fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = UuidGeneratorApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of V5n fields).
     */
     ...
     * Load a single V5n.
     *
     * @param V5nLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed V5nLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return V5n|array The loaded V5n as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = UuidGeneratorApi2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List V5n items matching the given filter.
     *
     * @param V5nListMatch|array|null $reqmatch Match filter (any subset
     *   of V5n fields) as an assoc-array; V5nListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return V5n[]|array A list of V5n items as assoc-arrays at
