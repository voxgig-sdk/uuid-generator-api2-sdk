...
// UuidGeneratorApi2 SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class UuidGeneratorApi2MakePoint
{
    public static function call(UuidGeneratorApi2Context $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof UuidGeneratorApi2Error) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = UuidGeneratorApi2Helpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = UuidGeneratorApi2Helpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
