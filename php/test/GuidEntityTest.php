...
class GuidEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set UUIDGENERATORAPI__TEST_GUID_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$guid_ref01_list_result, $err] = $guid_ref01_ent->list($guid_ref01_match, null);
        $this->assertNull($err);
        $this->assertIsArray($guid_ref01_list_result);

        // LOAD
        $guid_ref01_match_dt0 = [];
        [$guid_ref01_data_dt0_loaded, $err] = $guid_ref01_ent->load($guid_ref01_match_dt0, null);
        $this->assertNull($err);
...
function guid_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("UUIDGENERATORAPI__TEST_GUID_ENTID");
