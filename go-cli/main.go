// Command UuidGeneratorApi2-cli is a CLI + REPL for the UuidGeneratorApi2 SDK,
...
const prompt = "uuid-generator-api2"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "guid v1n v1n2 v3n v3n2 v4n v4n2 v5n v5n2 v6n v6n2 v7n v7n2"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      UUID_GENERATOR_API2_APIKEY (api key), UUID_GENERATOR_API2_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: UUID_GENERATOR_API2_APIKEY carries the API key and
	// UUID_GENERATOR_API2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("UUID_GENERATOR_API2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("UUID_GENERATOR_API2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUuidGeneratorApi2SDK(opts)
