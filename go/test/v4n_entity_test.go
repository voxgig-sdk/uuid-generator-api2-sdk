...
func TestV4nEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.V4n(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.V4n(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		// LOAD
		v4nRef01MatchDt0 := map[string]any{}
		v4nRef01DataDt0Loaded, err := v4nRef01Ent.Load(v4nRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if v4nRef01DataDt0Loaded == nil {
...
func v4nBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("UUID_GENERATOR_API2_TEST_V4N_ENTID")
	...
	idmapResolved := core.ToMapAny(env["UUID_GENERATOR_API2_TEST_V4N_ENTID"])
