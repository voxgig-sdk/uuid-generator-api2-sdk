...
func TestV5nEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.V5n(nil)
		if ent == nil {
			t.Fatal("expected non-nil V5nEntity")
		}
	})
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.V5n(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.V5n(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		v5nRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.v5n", setup.data)))
		var v5nRef01Data map[string]any
		if len(v5nRef01DataRaw) > 0 {
			v5nRef01Data = core.ToMapAny(v5nRef01DataRaw[0][1])
		}
		...
		// LIST
		v5nRef01Ent := client.V5n(nil)
		v5nRef01Match := map[string]any{}

		v5nRef01ListResult, err := v5nRef01Ent.List(v5nRef01Match, nil)
		if err != nil {
...
func v5nBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "v5n", "V5nTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read v5n test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse v5n test data: " + err.Error())
	}
