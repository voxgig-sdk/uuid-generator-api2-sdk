...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("UUIDGENERATORAPI2_TEST_LIVE") == "TRUE" ||
		os.Getenv("UUIDGENERATORAPI2_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("UUIDGENERATORAPI2_TEST_EXPLAIN"); explain != "" {
