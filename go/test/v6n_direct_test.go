...
func TestV6nDirect(t *testing.T) {
	t.Run("direct-list-v6n", func(t *testing.T) {
		setup := v6nDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-v6n", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-v6n", func(t *testing.T) {
		setup := v6nDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-v6n", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type v6nDirectSetupResult struct {
...
func v6nDirectSetup(mockres any) *v6nDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"UUID_GENERATOR_API2_TEST_V6N_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["UUID_GENERATOR_API2_TEST_V6N_ENTID"]; ok {
		...
		return &v6nDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &v6nDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
