...
func TestV1nEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.V1n(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.V1n(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		v1nRef01Match := map[string]any{}

		v1nRef01ListResult, err := v1nRef01Ent.List(v1nRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, v1nRef01ListOk := v1nRef01ListResult.([]any)
		if !v1nRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", v1nRef01ListResult)
		}
...
func v1nBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("UUID_GENERATOR_API2_TEST_V1N_ENTID")
	...
	idmapResolved := core.ToMapAny(env["UUID_GENERATOR_API2_TEST_V1N_ENTID"])
