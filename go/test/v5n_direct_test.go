...
func TestV5nDirect(t *testing.T) {
	t.Run("direct-list-v5n", func(t *testing.T) {
		setup := v5nDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-v5n", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-v5n", func(t *testing.T) {
		setup := v5nDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-v5n", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type v5nDirectSetupResult struct {
...
func v5nDirectSetup(mockres any) *v5nDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"UUID_GENERATOR_API2_TEST_V5N_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["UUID_GENERATOR_API2_TEST_V5N_ENTID"]; ok {
		...
		return &v5nDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &v5nDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
