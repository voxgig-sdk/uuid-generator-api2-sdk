...
import (
	...
	omni "github.com/voxgig-sdk/uuid-generator-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UuidGeneratorApi2Error); is {
				out := map[string]any{
					"name":    "UuidGeneratorApi2Error",
...
func omniCtx(arg any, client *sdk.UuidGeneratorApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UuidGeneratorApi2SDK); is {
			client = live
