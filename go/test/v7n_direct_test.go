...
func TestV7nDirect(t *testing.T) {
	t.Run("direct-list-v7n", func(t *testing.T) {
		setup := v7nDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-v7n", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-v7n", func(t *testing.T) {
		setup := v7nDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-v7n", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type v7nDirectSetupResult struct {
...
func v7nDirectSetup(mockres any) *v7nDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"UUID_GENERATOR_API2_TEST_V7N_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["UUID_GENERATOR_API2_TEST_V7N_ENTID"]; ok {
		...
		return &v7nDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &v7nDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
