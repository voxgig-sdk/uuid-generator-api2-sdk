...
func TestV1nDirect(t *testing.T) {
	t.Run("direct-list-v1n", func(t *testing.T) {
		setup := v1nDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-v1n", _mode); _shouldSkip {
			if _reason == "" {
...
func v1nDirectSetup(mockres any) *v1nDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"UUID_GENERATOR_API2_TEST_V1N_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["UUID_GENERATOR_API2_TEST_V1N_ENTID"]; ok {
