...
The Ruby SDK for the UuidGeneratorApi2 API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Guid(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.UuidGeneratorApi2(nil).load(
  { "id" => "test01" }, nil
)
