...
class GuidEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Guid(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Guid(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = guid_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "guid." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set UUID_GENERATOR_API2_TEST_GUID_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    guid_ref01_data = nil
    if guid_ref01_data_raw.length > 0
...
def guid_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "guid", "GuidTestData.json")
  ...
  entid_env_raw = ENV["UUID_GENERATOR_API2_TEST_GUID_ENTID"]
