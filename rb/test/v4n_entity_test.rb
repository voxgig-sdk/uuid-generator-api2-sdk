...
class V4nEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.V4n(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.V4n(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set UUID_GENERATOR_API2_TEST_V4N_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    # LOAD
    v4n_ref01_match_dt0 = {}
    v4n_ref01_data_dt0_loaded = v4n_ref01_ent.load(v4n_ref01_match_dt0, nil)
    assert !v4n_ref01_data_dt0_loaded.nil?
...
def v4n_basic_setup(extra)
  ...
  entid_env_raw = ENV["UUID_GENERATOR_API2_TEST_V4N_ENTID"]
