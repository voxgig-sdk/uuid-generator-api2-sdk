...
class V7nDirectTest < Minitest::Test
  def test_direct_list_v7n
    setup = v7n_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-v7n", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_v7n
    setup = v7n_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-v7n", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
