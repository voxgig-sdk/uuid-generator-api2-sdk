...
class V6nEntity
  ...
  def make
    opts = @_entopts.dup
    V6nEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UuidGeneratorApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [V6n, Hash] the current V6n data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UuidGeneratorApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of V6n fields)
  def match_get
  ...
  # Load a single V6n.
  #
  # @param reqmatch [V6nLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.V6n.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [V6n, Hash] the loaded V6n; raises UuidGeneratorApi2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UuidGeneratorApi2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [V6nListMatch, Hash, nil] match filter (any subset of
  #   V6n fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<V6n>, Array] the matching V6n items; raises UuidGeneratorApi2Error on failure
