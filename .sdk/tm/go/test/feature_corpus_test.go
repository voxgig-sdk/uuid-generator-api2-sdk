...
//
// NewUuidGeneratorApi2SDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.UuidGeneratorApi2SDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewUuidGeneratorApi2SDK(opts)
}
...
func fcCandidates(client *sdk.UuidGeneratorApi2SDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.UuidGeneratorApi2SDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.UuidGeneratorApi2SDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.UuidGeneratorApi2Error
