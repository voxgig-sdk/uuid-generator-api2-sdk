...
def make_error_util(ctx, err):
    if ctx is None:
        from projectname_sdk.core.context import UuidGeneratorApi2Context
        ctx = UuidGeneratorApi2Context({}, None)
    ...
    result = ctx.result
    if result is None:
        result = UuidGeneratorApi2Result({})
    result.ok = False
    ...
    errmsg = ""
    if isinstance(err, UuidGeneratorApi2Error):
        errmsg = err.msg
    ...
    sdk_err = UuidGeneratorApi2Error("", msg, ctx)
