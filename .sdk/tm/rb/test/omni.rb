...
module UuidGeneratorApi2Omni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if UuidGeneratorApi2Omni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = UuidGeneratorApi2Omni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| UuidGeneratorApi2Omni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || UuidGeneratorApi2Omni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = UuidGeneratorApi2Spec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = UuidGeneratorApi2Error.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { UuidGeneratorApi2Omni.errify(err) }
