...
module UuidGeneratorApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UuidGeneratorApi2Response.new({})
    ...
    if fetch_err
      ...
      response = UuidGeneratorApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UuidGeneratorApi2Response.new(fetched)
    else
