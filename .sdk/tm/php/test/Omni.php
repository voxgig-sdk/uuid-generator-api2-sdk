...
//    only - a typed UuidGeneratorApi2Context there would make every ctx
...
final class UuidGeneratorApi2Omni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new UuidGeneratorApi2Context($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new UuidGeneratorApi2Spec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new UuidGeneratorApi2Result($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new UuidGeneratorApi2Error('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new UuidGeneratorApi2Response($clean['response']);
    ...
    public static function err_from_map($m): ?UuidGeneratorApi2Error
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new UuidGeneratorApi2Error(is_string($code) ? $code : '', $msg);
    }
