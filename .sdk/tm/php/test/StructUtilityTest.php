...
// UuidGeneratorApi2 SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = UuidGeneratorApi2Omni::makeStructRunner(
    ...
    {
        return UuidGeneratorApi2Omni::objmodel(UuidGeneratorApi2Omni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return UuidGeneratorApi2Omni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return UuidGeneratorApi2Omni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(UuidGeneratorApi2Omni::structfix($now, Struct::undef()))
                        !== json_encode(UuidGeneratorApi2Omni::structfix($val, Struct::undef()))) {
