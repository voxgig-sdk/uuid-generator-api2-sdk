...
// UuidGeneratorApi2 SDK retry feature
...
class UuidGeneratorApi2RetryFeature extends UuidGeneratorApi2BaseFeature
{
    ...
    public function init(UuidGeneratorApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UuidGeneratorApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UuidGeneratorApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UuidGeneratorApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
