...
The Lua SDK for the UuidGeneratorApi2 API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Guid(nil):list(nil, nil)
...
```lua
local result, err = client:Guid(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:UuidGeneratorApi2(nil):load(
  { id = "test01" }, nil
)
