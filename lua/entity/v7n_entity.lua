...
local V7nEntity = {}
V7nEntity.__index = V7nEntity


function V7nEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V7nEntity)
  self._name = "v7n"
  self._client = client
...
function V7nEntity:get_name()
  return self._name
end


function V7nEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V7nEntity.new(self._client, opts)
end
...
function V7nEntity:mark_deleted()
  self._deleted = true
end


function V7nEntity:deleted()
  return true == self._deleted
end


function V7nEntity:data_set(args)
  if args ~= nil then
...
function V7nEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V7nEntity:match_set(args)
  if args ~= nil then
...
function V7nEntity:stream(action, args, callopts)
...
---@param reqmatch V7nLoadMatch
---@param ctrl? table
---@return V7n
---@return string? err
function V7nEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch V7nListMatch
---@param ctrl? table
---@return V7n[]
---@return string? err
function V7nEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function V7nEntity:_run_op(ctx, post_done)
  local utility = self._utility
