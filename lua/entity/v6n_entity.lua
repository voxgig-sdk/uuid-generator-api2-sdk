...
local V6nEntity = {}
V6nEntity.__index = V6nEntity


function V6nEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V6nEntity)
  self._name = "v6n"
  self._client = client
...
function V6nEntity:get_name()
  return self._name
end


function V6nEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V6nEntity.new(self._client, opts)
end
...
function V6nEntity:mark_deleted()
  self._deleted = true
end


function V6nEntity:deleted()
  return true == self._deleted
end


function V6nEntity:data_set(args)
  if args ~= nil then
...
function V6nEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V6nEntity:match_set(args)
  if args ~= nil then
...
function V6nEntity:stream(action, args, callopts)
...
---@param reqmatch V6nLoadMatch
---@param ctrl? table
---@return V6n
---@return string? err
function V6nEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch V6nListMatch
---@param ctrl? table
---@return V6n[]
---@return string? err
function V6nEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function V6nEntity:_run_op(ctx, post_done)
  local utility = self._utility
