...
local V5nEntity = {}
V5nEntity.__index = V5nEntity


function V5nEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V5nEntity)
  self._name = "v5n"
  self._client = client
...
function V5nEntity:get_name()
  return self._name
end


function V5nEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V5nEntity.new(self._client, opts)
end
...
function V5nEntity:mark_deleted()
  self._deleted = true
end


function V5nEntity:deleted()
  return true == self._deleted
end


function V5nEntity:data_set(args)
  if args ~= nil then
...
function V5nEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V5nEntity:match_set(args)
  if args ~= nil then
...
function V5nEntity:stream(action, args, callopts)
...
---@param reqmatch V5nLoadMatch
---@param ctrl? table
---@return V5n
---@return string? err
function V5nEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch V5nListMatch
---@param ctrl? table
---@return V5n[]
---@return string? err
function V5nEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function V5nEntity:_run_op(ctx, post_done)
  local utility = self._utility
