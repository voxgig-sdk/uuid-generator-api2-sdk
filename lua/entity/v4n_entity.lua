...
local V4nEntity = {}
V4nEntity.__index = V4nEntity


function V4nEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V4nEntity)
  self._name = "v4n"
  self._client = client
...
function V4nEntity:get_name()
  return self._name
end


function V4nEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V4nEntity.new(self._client, opts)
end
...
function V4nEntity:mark_deleted()
  self._deleted = true
end


function V4nEntity:deleted()
  return true == self._deleted
end


function V4nEntity:data_set(args)
  if args ~= nil then
...
function V4nEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V4nEntity:match_set(args)
  if args ~= nil then
...
function V4nEntity:stream(action, args, callopts)
...
---@param reqmatch V4nLoadMatch
---@param ctrl? table
---@return V4n
---@return string? err
function V4nEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch V4nListMatch
---@param ctrl? table
---@return V4n[]
---@return string? err
function V4nEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function V4nEntity:_run_op(ctx, post_done)
  local utility = self._utility
