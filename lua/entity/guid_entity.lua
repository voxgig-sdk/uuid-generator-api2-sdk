...
local GuidEntity = {}
GuidEntity.__index = GuidEntity


function GuidEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GuidEntity)
  self._name = "guid"
  self._client = client
...
function GuidEntity:get_name()
  return self._name
end


function GuidEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GuidEntity.new(self._client, opts)
end
...
function GuidEntity:mark_deleted()
  self._deleted = true
end


function GuidEntity:deleted()
  return true == self._deleted
end


function GuidEntity:data_set(args)
  if args ~= nil then
...
function GuidEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GuidEntity:match_set(args)
  if args ~= nil then
...
function GuidEntity:stream(action, args, callopts)
...
function GuidEntity:_run_op(ctx, post_done)
  local utility = self._utility
