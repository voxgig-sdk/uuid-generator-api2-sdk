...
describe("V7nDirect", function()
  it("should direct-list-v7n", function()
    local setup = v7n_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-v7n", setup.live and "live" or "unit")
  ...
  it("should direct-load-v7n", function()
    local setup = v7n_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-v7n", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
