...
describe("V1nEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:V1n(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:V1n(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set UUID_GENERATOR_API2_TEST_V1N_ENTID JSON to run live")
      return
    end
    ...
    local v1n_ref01_match = {}

    local v1n_ref01_list_result, err = v1n_ref01_ent:list(v1n_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(v1n_ref01_list_result)
...
function v1n_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("UUID_GENERATOR_API2_TEST_V1N_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["UUID_GENERATOR_API2_TEST_V1N_ENTID"])
