...
describe("GuidEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Guid(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Guid(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set UUID_GENERATOR_API2_TEST_GUID_ENTID JSON to run live")
      return
    end
...
function guid_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("UUID_GENERATOR_API2_TEST_GUID_ENTID")
